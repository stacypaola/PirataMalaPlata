...
@onready var _animation := $FantasmaAnimation
@onready var _animation_effect := $FantasmaEffect
...
func _turn():
	if not _raycast_terrain.is_colliding() or _raycast_wall.is_colliding():
		...
		if !_object or (_object && !_object.is_in_group("player")):
...
func _detection():
	...
	if _object1 && _object1.is_in_group("player") && _raycast_vision_left.is_colliding():
		_move(true)
	
	if _object2 && _object2.is_in_group("player") && _raycast_vision_right.is_colliding():
		_move(false)
	
	if !_object1 && !_object2 && _animation.get_animation() != "attack":
...
func _on_area_2d_area_entered(area):
	if area.is_in_group("hit") || area.is_in_group("die"):
		die = true
		_damage()
